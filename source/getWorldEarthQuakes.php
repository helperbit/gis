...
$startTime = pg_escape_string($_GET["startTime"]);
$endTime = pg_escape_string($_GET["endTime"]);
$magMin = pg_escape_string($_GET["mag"]);
$magMax = 10;
...
$stringaQuery = "SELECT row_to_json(fc)
     ...
     FROM (SELECT 'Feature' As type
       ...
       As lg where time > '$startTime' AND time < '$endTime' AND mag < $magMax AND mag > $magMin 
        ) As f )  As fc;";
