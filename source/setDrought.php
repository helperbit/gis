...
$link = "https://www.gdacs.org/datareport/resources/DR/";
$sec = 0;
$ch = curl_init();
...
// GET ALL THE ID_EVENTO
            ...
            $stringaQuery = "SELECT * FROM public.drought WHERE gdacs_id = '$id_gdacs'";
            ...
            if ($resultselect->rowCount() == 0) { // EVENTO NUOVO
                ...
                $linkgeojson = "/datareport/resources/DR/" . $end[1] . "/geojson_" . $end[1] . "_" . $last_episodio . ".geojson";
                ...
                $xml = getxml($preurl, "DR", ".xml", $end[1], $last_episodio, "/rss_");
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sql = "INSERT INTO public.drought(
...
try {
                        ...
                        $geometry = (string) $xml->info->area->polygon[0];
                        //incidents
                        ...
                        $sql = "INSERT INTO public.drought( gdacs_id, created_at, crisis_alert_level, crisis_event_episode, crisis_population, crisis_severity,  crisis_vulnerability, dc_date, dc_description, dc_title, gn_parent_country, geom)
                VALUES ( ? , ?, ? ,  ?,  ?,  ?,  ?,  ?,  ?,  ?,  ?,  ST_Centroid(st_setsrid(ST_FlipCoordinates(ST_MakePolygon(ST_GeomFromText(?))),4326)))";
                        $params = array(
                            ...
                            print "\n";
                            print "NON E STATO POSSIBILE RICAVARE I DATI DAL FILE XML : PARAMETRI MANCANTI";
                            print "\n";
                    ...
                    if ($todate >= $controldate) {
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($stmt->execute($params)) {
                                echo "INSERITO IN drought $gdacs_id WITH EVENT EPISODE : $event_episode <br>";
                                email_sender("NEW SYSTEM: Creato evento drought", "Creato evento drought $gdacs_id .");
                                ...
                                $updateSql = 'UPDATE public.drought
                            ...
                            where gdacs_id = ?';
                                ...
                                foreach ($features as $feature) {
                                    ...
                                    if ($geometry['type'] == "Polygon" || $geometry['type'] == "MultiPolygon") {
                                        ...
                                        //$features = $obj['features'];
                                        $geominser_query = "INSERT INTO public.drought_area(
                                        ...
                                        if ($stmt->execute($params)) {
                                            ...
                                            print "\n";
                                            print_r('NEW polygon inserted into drought_area ' . " --> " . $gdacs_id . " with alert level -> " . $polygontype);
                                            print "\n";
                                        } else {

                                            print "\n";
                                            print_r(' --> IMPOSSIBILE INSERIRE CICLONI IN  DROUGHT AREA ' . $gdacs_id . " with alert level -> " . $polygontype);
                                            print "\n";
                                            print_r($stmt->errorInfo());
                                            print "\n";
                                        }
              ...
              /*  print "\n";
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sqlupdate_test = "UPDATE public.drought
                      ...
                      try {
                        ...
                        $tdate = $parametri['tdate'];
                        $created_at = $parametri['created_at'];
                        $crisis_alert_level = $parametri['crisis_alertLevel'];
                        $event_episode = $parametri['event_episode'];
                        $crisis_population = $parametri['crisis_population'];
                        $crisis_severity = $parametri['crisis_severity'];
                        $crisis_vulnerability = $parametri['crisis_vulnerability'];
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($last_episodio > $crisis_event_episode) {
                              ...
                              if ($stmt00->execute($params)) {
                                  print "\n";
                                  print_r(' --> AGGIORNATO DROUGHT  TEST ' . $gdacs_id);
                                  print "\n";
                                  email_sender("NEW SYSTEM: Aggiornato evento drought", "Aggiornato evento drought $gdacs_id .");
                              } else {
                                  print "\n";
                                  print "\n";
                                  print_r(' --> IMPOSSIBILE AGGIORNARE DROUGHT  TEST ' . $gdacs_id);
                                  print "\n";
                              }
                                ...
                                $stringadelete = "DELETE FROM public.drought_area
                              	WHERE gdacs_id = ?";

                                $stmtdelete = $connection->prepare($stringadelete);
                                if ($stmtdelete->execute(array($gdacs_id))) {
                                    print "\n";
                                    print_r(' --> CANCELLATI EVENTI IN  DROUGHT AREA');
                                    print "\n";
                                    $features = $obj['features'];
                                    ...
                                    foreach ($features as $feature) {
                                        ...
                                        if ($geometry['type'] == "Polygon" || $geometry['type'] == "MultiPolygon") {
                                            ...
                                            if ($stmt->execute($params)) {
                                                print "\n";
                                                print_r(' --> INSERITI DROUGHT IN  DROUGHT AREA ' . $gdacs_id . " with alert level -> " . $polygontype);
                                                print "\n";
                                            } else {
                                                print "\n";
                                                print "\n";
