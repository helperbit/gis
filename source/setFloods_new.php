...
$link = "https://www.gdacs.org/datareport/resources/FL/";
$sec = 0;
$ch = curl_init();
...
// GET ALL THE ID_EVENTO
            ...
            $stringaQuery = "SELECT * FROM public.floods_test WHERE gdacs_id = '$id_gdacs'";
            ...
            if ($resultselect->rowCount() == 0) { // EVENTO NUOVO
                ...
                $linkgeojson = "/datareport/resources/FL/" . $end[1] . "/geojson_" . $end[1] . "_" . $last_episodio . ".geojson";
                ...
                $xml = getxml($preurl, "FL", ".xml", $end[1], $last_episodio, "/rss_");
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sql = "INSERT INTO public.floods_test(
...
try {
                        ...
                        $sql = "INSERT INTO public.floods_test( gdacs_id, created_at, crisis_alert_level, crisis_event_episode, crisis_population, crisis_severity,  crisis_vulnerability, dc_date, dc_description, dc_title, gn_parent_country, geom)
                    ...
                    if ($todate >= $controldate) {
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($stmt->execute($params)) {
                                echo "INSERITO IN floods_test $gdacs_id WITH EVENT EPISODE : $event_episode <br>";
                                ...
                                $updateSql = 'UPDATE public.floods_test
                            ...
                            where gdacs_id = ?';
                                ...
                                foreach ($features as $feature) {
                                    ...
                                    if ($geometry['type'] == "Polygon" || $geometry['type'] == "MultiPolygon") {
                                        ...
                                        //$features = $obj['features'];
                                        $geominser_query = "INSERT INTO public.floods_area(
                                        ...
                                        if ($stmt->execute($params)) {
                                            ...
                                            print "\n";
                                            print_r('NEW polygon inserted into floods_area ' . " --> " . $gdacs_id . " with alert level -> " . $polygontype);
                                            print "\n";
                                        } else {
              ...
              /*  print "\n";
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sqlupdate_test = "UPDATE public.floods_test
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($last_episodio > $crisis_event_episode) {
                              ...
                              if ($stmt00->execute($params)) {
                                  print "\n";
                                  print_r(' --> AGGIORNATO FLOOD  TEST ' . $gdacs_id);
                                  print "\n";
                              } else {
                                  print "\n";
                                  print "\n";
                                  print_r(' --> IMPOSSIBILE AGGIORNARE FLOOD  TEST ' . $gdacs_id);
                                  print "\n";
                              }
                                ...
                                $stringadelete = "DELETE FROM public.floods_area
                              	WHERE gdacs_id = ?";
