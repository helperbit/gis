...
$stringaQuery = "SELECT row_to_json(fc)
     ...
     FROM (SELECT 'Feature' As type
        , ST_AsGeoJSON(st_transform(lg.max_epicentro,4326))::json As geometry
       ...
       old_dam_dollars as danni_in_milioni_dollari_ultimo_evento ,
       ne_10m_time_zones.zone,
       usgs_name_all
        )
       ...
       left join  (select distinct on (eventi_terremoto.id_evento, old_earthquakes.id,old_data_evento) eventi_terremoto.id_evento, true as old_event,
  ...
  and max_magnitude < old_mag and EXTRACT(YEAR FROM old_data_evento::date) < 2014 order by eventi_terremoto.id_evento, old_earthquakes.id,old_data_evento DESC ) as ciao
