...
$link = "https://www.gdacs.org/datareport/resources/VW/";
$sec = 0;
$ch = curl_init();
...
// GET ALL THE ID_EVENTO
            ...
            $stringaQuery = "SELECT * FROM public.windstorms WHERE gdacs_id = '$id_gdacs'";
            ...
            if ($resultselect->rowCount() == 0) { // EVENTO NUOVO
                ...
                $linkgeojson = "/datareport/resources/VW/" . $end[1] . "/geojson_" . $end[1] . "_" . $last_episodio . ".geojson";
                ...
                $xml = getxml($preurl, "VW", ".xml", $end[1], $last_episodio, "/rss_");
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sql = "INSERT INTO public.windstorms(
                 gdacs_id, created_at, crisis_alert_level, crisis_event_episode, crisis_population, crisis_severity, crisis_severity_hash, crisis_vulnerability, crisis_vulnerability_hash, dc_date, dc_description, dc_title, gn_parent_country, geom)
                  VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, st_setsrid(ST_MakePoint(?,?),4326));";
                        ...
                        $sql = "INSERT INTO public.windstorms( gdacs_id, created_at, crisis_alert_level, crisis_event_episode, crisis_population, crisis_severity,  crisis_vulnerability, dc_date, dc_description, dc_title, gn_parent_country, geom)
                    ...
                    if ($todate >= $controldate) {
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($stmt->execute($params)) {
                                echo "INSERITO IN windstorms $gdacs_id WITH EVENT EPISODE : $event_episode <br>";
                                //email_sender("NEW SYSTEM: Creato evento flood", "Creato evento floods $crisis_eventid .");
                                //update
                                email_sender("NEW SYSTEM: Creato evento windstorm", "Creato evento windstorm $gdacs_id .");
                                ...
                                $updateSql = 'UPDATE public.windstorms
                            ...
                            where gdacs_id = ?';
                                ...
                                foreach ($features as $feature) {
                                    ...
                                    if ($geometry['type'] == "Polygon" || $geometry['type'] == "MultiPolygon") {
                                        ...
                                        //$features = $obj['features'];
                                        $geominser_query = "INSERT INTO public.windstorms_area(
                                        ...
                                        if ($stmt->execute($params)) {
                                            ...
                                            print "\n";
                                            print_r('NEW polygon inserted into windstorms_area ' . " --> " . $gdacs_id . " with alert level -> " . $polygontype);
                                            print "\n";
                                        } else {
              ...
              /*  print "\n";
                ...
                if ($xml) {
                    if ($xml->channel) {
                        ...
                        $sqlupdate_test = "UPDATE public.windstorms
                        ...
                        if ($crisis_alertLevel != "Green") {
                            ...
                            if ($last_episodio > $crisis_event_episode) {
                              ...
                              if ($stmt00->execute($params)) {
                                  ...
                                  print "\n";
                                  email_sender("NEW SYSTEM: Aggiornato evento windstorm", "Aggiornato evento windstorm $gdacs_id .");
                              } else {
                                  print "\n";
                                  print "\n";
                                ...
                                $stringadelete = "DELETE FROM public.windstorms_area
                              	WHERE gdacs_id = ?";
